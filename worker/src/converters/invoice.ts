...
export function stripeInvoiceToNotionProperties(
  ...
  const properties: Record<string, any> = {
    ...
    "Latest Revision": createRichTextProperty(typeof invoice.latest_revision === 'string' ? invoice.latest_revision : invoice.latest_revision?.id || null),
  ...
  if (invoice.automatic_tax?.disabled_reason) {
    properties["Automatic Tax Disabled Reason"] = createSelectProperty(invoice.automatic_tax.disabled_reason);
  }

  properties["Automatic Tax Provider"] = createRichTextProperty(invoice.automatic_tax?.provider);
  properties["Automatically Finalizes At"] = createDateProperty(invoice.automatically_finalizes_at);
  ...
  if (invoice.from_invoice) {
    properties["From Invoice Action"] = createRichTextProperty(invoice.from_invoice.action);
    properties["From Invoice ID"] = createRichTextProperty(typeof invoice.from_invoice.invoice === 'string' ? invoice.from_invoice.invoice : invoice.from_invoice.invoice?.id || null);
  }

  // Handle default tax rates
  const defaultTaxRatesCount = invoice.default_tax_rates?.length || 0;
  properties["Default Tax Rates Count"] = createNumberProperty(defaultTaxRatesCount);
  if (invoice.default_tax_rates && invoice.default_tax_rates.length > 0) {
    const taxRates = invoice.default_tax_rates.map(rate => {
  ...
  properties["Customer Tax IDs Count"] = createNumberProperty(customerTaxIdsCount);
  if (invoice.customer_tax_ids && invoice.customer_tax_ids.length > 0) {
    const taxIds = invoice.customer_tax_ids.map(taxId => 
  ...
  properties["Account Tax IDs Count"] = createNumberProperty(accountTaxIdsCount);
  ...
  // Handle totals
  properties["Subtotal Excluding Tax"] = createNumberProperty(invoice.subtotal_excluding_tax);
  properties["Total Excluding Tax"] = createNumberProperty(invoice.total_excluding_tax);
  properties["Pre Payment Credit Notes Amount"] = createNumberProperty(invoice.pre_payment_credit_notes_amount);
  properties["Post Payment Credit Notes Amount"] = createNumberProperty(invoice.post_payment_credit_notes_amount);

  // Handle total discount amounts
  const totalDiscountAmountsCount = invoice.total_discount_amounts?.length || 0;
  properties["Total Discount Amounts Count"] = createNumberProperty(totalDiscountAmountsCount);
  if (invoice.total_discount_amounts && invoice.total_discount_amounts.length > 0) {
    const totalDiscounts = invoice.total_discount_amounts.map(discount => 
      `${discount.amount} (${discount.discount})`
    ).join(', ');
    properties["Total Discount Amounts"] = createRichTextProperty(totalDiscounts);
  } else {
    properties["Total Discount Amounts"] = createRichTextProperty("");
  }

  // Handle total pretax credit amounts
  const totalPretaxCreditAmountsCount = invoice.total_pretax_credit_amounts?.length || 0;
  properties["Total Pretax Credit Amounts Count"] = createNumberProperty(totalPretaxCreditAmountsCount);
  if (invoice.total_pretax_credit_amounts && invoice.total_pretax_credit_amounts.length > 0) {
    const totalPretaxCredits = invoice.total_pretax_credit_amounts.map(credit => {
    ...
    properties["Total Pretax Credit Amounts"] = createRichTextProperty(totalPretaxCredits);
  } else {
    properties["Total Pretax Credit Amounts"] = createRichTextProperty("");
  }
  ...
  if (invoice.total_taxes && invoice.total_taxes.length > 0) {
    const totalTaxes = invoice.total_taxes.map(tax => {
  ...
  if (invoice.shipping_cost) {
    properties["Shipping Cost Amount Subtotal"] = createNumberProperty(invoice.shipping_cost.amount_subtotal);
    properties["Shipping Cost Amount Tax"] = createNumberProperty(invoice.shipping_cost.amount_tax);
    properties["Shipping Cost Amount Total"] = createNumberProperty(invoice.shipping_cost.amount_total);
    properties["Shipping Cost Shipping Rate"] = createRichTextProperty(stringFromObject(invoice.shipping_cost.shipping_rate));
  }

  // Handle shipping details
  if (invoice.shipping_details) {
    properties["Shipping Details Name"] = createRichTextProperty(invoice.shipping_details.name);
    properties["Shipping Details Phone"] = createPhoneProperty(invoice.shipping_details.phone);
    
    if (invoice.shipping_details.address) {
      properties["Shipping Details Address Line 1"] = createRichTextProperty(invoice.shipping_details.address.line1);
      properties["Shipping Details Address Line 2"] = createRichTextProperty(invoice.shipping_details.address.line2);
      properties["Shipping Details City"] = createRichTextProperty(invoice.shipping_details.address.city);
      properties["Shipping Details State"] = createRichTextProperty(invoice.shipping_details.address.state);
      properties["Shipping Details Postal Code"] = createRichTextProperty(invoice.shipping_details.address.postal_code);
      properties["Shipping Details Country"] = createRichTextProperty(invoice.shipping_details.address.country);
    }
  }

  // Handle threshold reason
  if (invoice.threshold_reason) {
    properties["Threshold Reason Amount GTE"] = createNumberProperty(invoice.threshold_reason.amount_gte);
    properties["Threshold Reason Item Reasons Count"] = createNumberProperty(invoice.threshold_reason.item_reasons?.length || 0);
  }

  // Handle issuer
  if (invoice.issuer) {
    properties["Issuer Type"] = createSelectProperty(invoice.issuer.type);
    properties["Issuer Account"] = createRichTextProperty(stringFromObject(invoice.issuer.account));
  }

  // Handle confirmation secret
  if (invoice.confirmation_secret) {
    properties["Confirmation Secret Type"] = createRichTextProperty(invoice.confirmation_secret.type);
    properties["Confirmation Secret Client Secret"] = createRichTextProperty(invoice.confirmation_secret.client_secret);
  }

  // Handle last finalization error
  if (invoice.last_finalization_error) {
    properties["Last Finalization Error Type"] = createRichTextProperty(invoice.last_finalization_error.type);
    properties["Last Finalization Error Code"] = createRichTextProperty(invoice.last_finalization_error.code);
    properties["Last Finalization Error Message"] = createRichTextProperty(invoice.last_finalization_error.message);
  }

  // Handle payment method types
  const paymentMethodTypesCount = invoice.payment_settings?.payment_method_types?.length || 0;
  properties["Payment Method Types Count"] = createNumberProperty(paymentMethodTypesCount);
  if (invoice.payment_settings?.payment_method_types && invoice.payment_settings.payment_method_types.length > 0) {
    const paymentMethodTypes = invoice.payment_settings.payment_method_types.join(', ');
    properties["Payment Method Types"] = createRichTextProperty(paymentMethodTypes);
  } else {
