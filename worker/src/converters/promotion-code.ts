...
export function stripePromotionCodeToNotionProperties(promotionCode: Stripe.PromotionCode, customerNotionPageId: string | null) {
  const properties: Record<string, any> = {
    ...
    "Metadata": createRichTextProperty(JSON.stringify(promotionCode.metadata || {})),
  };
  ...
  if (promotionCode.coupon) {
    ...
    properties["Coupon Duration In Months"] = createNumberProperty(coupon.duration_in_months);

    properties["Coupon Amount Off"] = createNumberProperty(coupon.amount_off);

    properties["Coupon Percent Off"] = createNumberProperty(coupon.percent_off ? coupon.percent_off / 100 : null);

    properties["Coupon Currency"] = createRichTextProperty(coupon.currency?.toUpperCase());

    properties["Coupon Valid"] = createCheckboxProperty(coupon.valid);

    properties["Coupon Times Redeemed"] = createNumberProperty(coupon.times_redeemed || 0);

    properties["Coupon Max Redemptions"] = createNumberProperty(coupon.max_redemptions);
    ...
    if (coupon.applies_to?.products && coupon.applies_to.products.length > 0) {
      properties["Coupon Applies To Products"] = createRichTextProperty(coupon.applies_to.products.join(", "));
    } else {
    ...
    if (coupon.currency_options && Object.keys(coupon.currency_options).length > 0) {
      properties["Coupon Currency Options Count"] = createNumberProperty(Object.keys(coupon.currency_options).length);
    } else {
  ...
  if (promotionCode.restrictions) {
    properties["Restrictions First Time Transaction"] = createCheckboxProperty(promotionCode.restrictions.first_time_transaction);

    properties["Restrictions Minimum Amount"] = createNumberProperty(promotionCode.restrictions.minimum_amount);

    properties["Restrictions Minimum Amount Currency"] = createRichTextProperty(promotionCode.restrictions.minimum_amount_currency?.toUpperCase());
    ...
    if (promotionCode.restrictions.currency_options && Object.keys(promotionCode.restrictions.currency_options).length > 0) {
      properties["Restrictions Currency Options Count"] = createNumberProperty(Object.keys(promotionCode.restrictions.currency_options).length);
    } else {
      properties["Restrictions Currency Options Count"] = createNumberProperty(0);
    }
  } else {
    properties["Restrictions First Time Transaction"] = createCheckboxProperty(false);
